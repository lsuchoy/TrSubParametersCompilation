...
def find_overriding_and_subducting_plates(subduction_shared_sub_segment):
    ...
    #if len(sharing_resolved_topologies) != 2: ########  REMOVED SECTION - LIOR  #########
    if len(sharing_resolved_topologies) == 0: ########  NEW SECTION - LIOR  #########
        return([])
    ...
    #for index in range(2): ########  REMOVED SECTION - LIOR  #########
    for index in range(len(sharing_resolved_topologies)): ########  NEW SECTION - LIOR  #########
    ...
    ########  END REMOVED SECTION - LIOR  #########
    
    ########  NEW SECTION - LIOR  #########
    if (overriding_plate is None) and (subducting_plate is None):
        return([])
    
    if subducting_plate is None:
        overriding_plate_id = overriding_plate.get_feature().get_reconstruction_plate_id()
        subducting_plate_id = overriding_plate_id
    elif overriding_plate is None:
        subducting_plate_id = subducting_plate.get_feature().get_reconstruction_plate_id()
        overriding_plate_id = subducting_plate_id
    else:
        overriding_plate_id = overriding_plate.get_feature().get_reconstruction_plate_id()
        subducting_plate_id = subducting_plate.get_feature().get_reconstruction_plate_id()

    return ([overriding_plate_id, subducting_plate_id, subduction_polarity])
    ########  END NEW SECTION - LIOR  #########
    ...
    * - extra data can be appended by specifying optional keyword arguments (*kwargs* - see list of options below).
    ...
    for shared_boundary_section in shared_boundary_sections:
        ...
        for shared_sub_segment in shared_boundary_section.get_shared_sub_segments():
            ...
            if not overriding_and_subducting_plates:
                warnings.warn('Unable to find the overriding and subducting plates of the subducting sub-segment "{0}" at {1}Ma.\n'
                              '    Either the subduction polarity is not properly set or the plateID of the adjacent plates is unreachable.\n'
                              '    Ignoring current sub-segment.'.format(
                                  shared_sub_segment.get_feature().get_name(), time),
                              RuntimeWarning)
                continue
            overriding_plate_id, subducting_plate_id, subduction_polarity = overriding_and_subducting_plates
            ########  END NEW SECTION - LIOR  #########
